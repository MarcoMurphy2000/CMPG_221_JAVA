...
public class MyLinkedList<E> 
{
	...
	After creating a list containing objects of my spaceshp class created in I_DO 1, 
	I wanted to sort this list according to the objects (Spaceships) max accelerations.
	Therefore I created this method below named sortSpaceships.
	
	*/
	
	public MyLinkedList sortSpaceships()
	{
		Node<E> ptr = head;
		Node<E> firstptr = null;
		Node<E> secondptr = null;
		...
		if (ptr == null){
			...
			{
				sortedlist.append(ptr.element); //Place calling list in sortedlist that can be changed instead of changing the calling list
			}
			ptr = head;  //reset pointer to head of original list
			
			//Sort list of spaceships
			firstptr = sortedlist.head;
			secondptr = firstptr.next;
			for (ptr = head; ptr != null; ptr = ptr.next)
			{
				while (secondptr != null)
				{
					if(((Comparable)firstptr.element).compareTo(secondptr.element)>0)
					{
						Node<E> holdNode = new Node<>(secondptr.element);
						secondptr.element = firstptr.element;
						firstptr.element = holdNode.element;
					}
					//move pointers forward and compare again until secondptr = null.
					firstptr = firstptr.next;
					secondptr = secondptr.next;
				}
				//reset pointers to first 2 elements in the list
				firstptr = sortedlist.head;
				secondptr = firstptr.next;
			}
			return sortedlist;
	...
	/** This is just another way of creating my sortSpaceships method using a counter. Basically just a bit more code involved.
