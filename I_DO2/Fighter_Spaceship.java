...
public class Fighter_Spaceship extends Spaceship
{
	...
	{
		return "Fighter spaceship with " + getLaserBlasters() + " laser blasters and " + getRocketLaunchers() + " rocket launchers on board has a max accelaration of: " + MaxAccelaration() + " Newton";
	}
	...
	public int compareTo(Spaceship s) 
	{
		...
		{
			Fighter_Spaceship FS = (Fighter_Spaceship)s;   
			if (this.MaxAccelaration() == FS.MaxAccelaration())
			{
				return 0;
			}
			else if (this.MaxAccelaration() < FS.MaxAccelaration())
			{
				return -1;
			}
			else
